function [finalField] = generateMines(heightMines,widthMines,totalMines,initialRow,initialCol)
...
for i = 1:totalMines
    ...
    while filled == false
        ...
        %if same cell as first pick, regenerate randi
        if randHeight == initialRow && randWidth == initialCol
            filled = false;
        %if it's a mine, regenerate randi
        elseif minefield(randHeight,randWidth) == 9
            filled = false;
        %if empty, make a mine
